...
RGBID_SLAM::PoseGraph::buildGraph(const std::vector<Pose> &poses, const std::vector<PoseConstraint> &constraints)   
{
  ...
  for (std::vector<Pose>::const_iterator p_it = poses.begin();
       p_it != poses.end(); p_it++)
  {    
    g2o::SE3Quat trafo((*p_it).rotation_.cast<double>(), (*p_it).translation_.cast<double>());
  ...
  {      
    ...
    g2o::EdgeSE3* eSE3 = new g2o::EdgeSE3();
    eSE3->vertices()[0] = optimizer_.vertex(from);
    eSE3->vertices()[1] = optimizer_.vertex(to);
    
    g2o::SE3Quat trafo((*c_it).rotation_.cast<double>(), (*c_it).translation_.cast<double>());
...
RGBID_SLAM::PoseGraph::updatePosesAndKeyframes(std::vector<Pose> &poses, std::vector<KeyframePtr> &keyframes)    
{
  ...
  for (std::vector<Pose>::iterator p_it = poses.begin();
       p_it != poses.end(); p_it++)
  {
    g2o::VertexSE3* vSE3 = dynamic_cast<g2o::VertexSE3*>(optimizer_.vertex((*p_it).id_));
    if (vSE3 != 0)
    {
  ...
  {
    g2o::VertexSE3* vSE3 = dynamic_cast<g2o::VertexSE3*>(optimizer_.vertex((*kf_it)->id_));
    if (vSE3 != 0)
    {
